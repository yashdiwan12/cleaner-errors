const PATTERNS = [
    {
        ...
        what: ([, nullish, prop]) => `You called .${prop} on a value that was ${nullish} at runtime. The variable wasn't populated before you used it.`,
    ...
    },
    {
        ...
        what: ([, name]) => `"${name}" is not a function at this point — it may be undefined, null, or a different type than expected.`,
    ...
    },
    {
        ...
        what: () => "A network request failed. The target server is unreachable, the URL is wrong, or there's no internet connection.",
    ...
    },
    {
        ...
        what: () => "A function is calling itself recursively without ever hitting a base case, causing a stack overflow.",
...
export function parseGenericError(err) {
    const e = err;
...
function extractLocationFromStack(stack) {
    ...
    const appLine = lines.find((l) => l.includes("at ") &&
