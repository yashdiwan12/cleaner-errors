 ...
 */
export function parse(err: unknown): EnhancedError {
  if (isZodError(err)) return parseZodError(err);
  if (isPrismaError(err)) return parsePrismaError(err);
 ...
 * }
 */
export function enhance(err: unknown): unknown {
 ...
 * await safeRun();
 */
export function withEnhance<T>(fn: () => Promise<T>): () => Promise<T> {
  return async () => {
    try {
      return await fn();
    } catch (err) {
